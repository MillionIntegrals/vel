...
def wrapped_env_maker(environment_id, seed, serial_id, disable_reward_clipping=False, monitor=True):
...
class ClassicAtariEnv(EnvFactoryBase):
    ...
    def __init__(self, envname, disable_reward_clipping=False, monitor=True):
    ...
    def instantiate(self, seed=0, serial_id=0) -> gym.Env:
        """ Make a single environment compatible with the experiments """
        return wrapped_env_maker(
            self.envname, seed, serial_id, disable_reward_clipping=self.disable_reward_clipping, monitor=self.monitor
        )
